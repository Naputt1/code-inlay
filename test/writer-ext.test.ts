...
import { applyInnerMarkers } from "../src/go-writer.js";
    ...
    const patches: GeneratedFilePatch[] = [
      {
        path: "a.go",
        regions: [
          { id: "dup", content: "x", owner: "test", language: "go", stableHash: "h1" },
        ],
      },
      {
        path: "b.go",
        regions: [
          { id: "dup", content: "y", owner: "test", language: "go", stableHash: "h2" },
        ],
      },
    ];
    const result = validateBeforeWrite(patches, diagnostics);
...
describe("applyInnerMarkers", () => {
  it("returns new body verbatim when neither has markers", () => {
    const region: GeneratedRegion = { id: "r1", content: "", owner: "test", language: "go", stableHash: "h1" };
