...
function toCleanContent(path: string, regions: Region[]): string {
  ...
  const result = pkg ? `package ${pkg}\n\n${withImports}${body}\n` : `${body}\n`;
  const hasUserEditableRegions = regions.some((r) => r.expectsUserCode || r.isStub);
  if (!hasUserEditableRegions && !result.startsWith("// Code generated by schemago.")) {
    const pkgMatch = result.match(/^package\s+\w+/m);
    if (pkgMatch) {
      const genHeader = [
  ...
  it("generates expected output for complex API config", { timeout: 60000 }, async () => {
    ...
    const genFiles = result.generation.files.map((f) => ({
      ...
      regions: f.regions.map((r) => ({
        ...
        kind: r.kind,
        imports: r.imports,
        expectsUserCode: r.expectsUserCode,
        isStub: r.isStub,
      })),
    }));
